...
KeyedCursorPrototype.remove = KeyedCursorPrototype[
  'delete'
 ...
 */
KeyedCursorPrototype.__iterate = IndexedCursorPrototype.__iterate = function(
  fn,
  reverse
) {
  var cursor = this
  var deref = cursor.deref()
  return deref && deref.__iterate
    ? deref.__iterate(function(v, k) {
 ...
 */
KeyedCursorPrototype.__iterator = IndexedCursorPrototype.__iterator = function(
  type,
  reverse
) {
  ...
  var cursor = this
  var iterator =
    deref && deref.__iterator && deref.__iterator(Iterator.ENTRIES, reverse)
...
function valToKeyPath(val) {
  return Array.isArray(val)
    ? val
    : Iterable.isIterable(val) ? val.toArray() : [val]
}
