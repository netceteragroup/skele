...
export default createStore => (reducer, initialAppState) => {
  ...
  const effectExecutor = callback => {
    console.log('in effectExecutor, executeEffects: ', executeEffects);
    if (executeEffects) {
      setTimeout(() => {
        if (wrappedDispatch) {
          ...
          console.warn(
            'There\'s beefn attempt to execute effects ' +
  ...
  wrappedDispatch = (...args) => callWithEffects(() => {
    store.dispatch(...args)
  });
  ...
  return {
    ...
    replaceReducer: nextReducer => store
      .replaceReducer((appState, action) => {
        return nextReducer(appState, { ...action, effectExecutor })
      })
  };
};
